...
export default function CoordinatorDashboard() {
    ...
    async function fetchData() {
      try {
        const [plansRes, marksRes, assignmentsRes, timetablesRes] = await Promise.all([
          fetch("http://localhost:3000/api/coordinator/plans", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://localhost:3000/api/coordinator/marks", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://localhost:3000/api/coordinator/assignments", { headers: { Authorization: `Bearer ${token}` } }),
          fetch("http://localhost:3000/api/coordinator/schedules", { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  async function handleReview(type, id, status, feedback = "") {
    try {
      const res = await fetch(`http://localhost:3000/api/coordinator/review/${type}/${id}`, {
        method: "PUT",
        headers: {
